...
KV Cache 从"推理引擎内部的临时张量"演变为"需要全局调度的 CDN 资产"，并非单纯因为显存不够，而是由**推理经济学、长上下文性能瓶颈、单集群方案的物理天花板、全局统计复用的网络规律**四股力量共同决定的：

1. **经济学驱动：Prefill 是算力吞噬者，"能少算就少算"**。对含系统提示词、RAG 知识库或长文档的请求，Prefill 的 FLOPs 远超 Decode；在高端 GPU 昂贵的现实里，让不同用户、不同请求重复计算相同公共前缀是对昂贵算力的极大浪费（重复 Prefill 浪费 30–70%）。复用 KV Cache 是降低 TCO 的唯一出路。
2. **性能驱动：长上下文爆发，TTFT 成为致命灾难**。上下文窗口 4K→128K→1M+，Prefill 耗时从毫秒级跃升到数秒甚至数十秒（100K token 在 8×H100 上也需约 10 秒），彻底摧毁交互式应用体验。唯一解法是"空间换时间"——提前算好并缓存；当 KV 体积达数十 GB，它就是一个需预热、跨网分发、就近读取的"大文件"。
3. **局部方案失效：单集群前缀复用是"漏水的池子"**。单集群内（vLLM/SGLang）的前缀缓存撞上三堵墙：显存容量墙（并发一大撑爆 HBM 只能拒服务）、冷热驱逐墙（LRU 把刚算好的热门 KV 驱逐，陷"计算—驱逐—再计算"死循环）、数据孤岛墙（集群 A 算过的前缀集群 B 无法复用，跨不过物理边界）。
4. **网络与统计学驱动：全局统计复用与时空套利**。突破单集群后，多区域/多租户的长尾需求汇聚成"全局热点"，大数定律最大化统计命中率；叠加全球算力成本差 2–5 倍与时区潮汐，可在算力便宜的"夜间源站"集中 Prefill、分发到离用户最近的"白天边缘"Decode，实现全球算力与能源的 CDN 级调度。

综上，当 KV Cache 同时具备极高的重算成本、GB 级体积、强烈的就近读取需求（低 TTFT）与全局统计复用价值时，其核心特征与 1990 年代静态网页、2010 年代高清流媒体完全同构。**回源代价的不对称性**尤其关键：传统 CDN 未命中只消耗带宽，KV CDN 未命中要消耗昂贵 GPU 算力与数十秒时间——这让"缓存命中"的经济学价值更强，也彻底确立了 KV Cache 走向 CDN 化的必然性。
