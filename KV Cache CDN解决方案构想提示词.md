我想输出一个完整的KV Cache CDN的解决方案构想报告，要求包含如下内容：

1. 核心结论（业界研究热点，技术演进方向 什么时候从远端拉取KV比本地重算更合适，技术可行性和商业可行性分析含计算过程） 

2. 解决方案构想：涉及到的部件，每个部件应该承担的职责，解决方案架构图，关键流程的逻辑数据流图

3. 背景：为什么KV Cache变成了CDN的问题 

4. KV Cache发展的五个时代 

5. KV Cache的四轴分类学：存哪里，生命周期，所有权，用什么存和用什么传 

6. 关键论文研究 

7. 跨机房与 "潮汐 KV"：从 PrfaaS 到 Internet for KV Cache网络
   硬件层配套：NVMe、CXL、SmartNIC、Rubin CPX

8. 关键挑战与解决方案：每个挑战与对应的解决方案，成熟度，仍然未解决的难题
   当前开放问题与研究前沿 

9. 当前主流的推理框架对KV Cache CDN的支持程度 
...
2. KVCache_CDN_Solution_Vision_Report_2026.md 为主

3. KV Cache CDN 解决方案构想1.md 和 KV Cache CDN 解决方案构想2.md
