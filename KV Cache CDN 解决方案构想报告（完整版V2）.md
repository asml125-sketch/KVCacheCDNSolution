...
| 四 | 部署拓扑解耦 | 算存强耦合同机房 → 全球算存分离 | 中心生成高价值 KV 资产，低谷期批量推送边缘 | 边缘就近 Decode、免大 Prefill 算力池，全球扩张 CapEx 下降 |
...
|---|---|---|
| 内容属性 | 静态独立文件，可跨机器/厂商/版本读取 | **模型绑定张量——严格绑定模型权重/结构、tokenizer/prompt 模板/RoPE 位置策略、精度量化/张量布局/TP-PP、LoRA/系统版本/引擎实现，任一不匹配即不可流用** |
...
KV 并不能像网页那样自由流通——它严格绑定模型与工程上下文。为此，块级寻址必须携带一份**不可变的 KV 兼容性清单（Compatibility Manifest）**：内容寻址不再是「模型 × tokenizer × 量化 × 块内容」，而是「哈希(兼容性清单 × 块内容)」（§6.2 部件②）。回源代价不对称同样关键：传统 CDN 未命中的代价只是带宽，KV CDN 未命中的代价是昂贵的 GPU 计算——**这使缓存命中的经济学价值更强，也使错误容忍度更低**。此外，全球算力分布极度不均（北美/东亚密集 vs 全球海量闲置消费级 GPU），把 KV 视为 CDN 资产还能通过 P2P 网络"吸附"长尾算力，形成 DC 骨干 + P2P 毛细血管的双层结构（部件⑧，见 §6.2）。
...
- **机制**：API 网关维护一个轻量级的全局 KV 目录。当用户请求到达时，网关**不随机分配**节点、也不只按地理就近分配，而是把"KV 驻留位置"作为路由决策的一个**高权重因子（而非唯一硬约束）**：查得历史会话 KV 驻留在东京节点后，在综合考量下**优先**路由到东京——同时权衡持续 Decode 的 RTT、跨境合规与故障集中度（§2.4）。
...
亲和性是**软约束**；真正的**硬约束是用户体验（首字延迟 + 持续 Decode 的 RTT）与综合成本**。把新加坡用户路由到东京，省下一次 Prefill，却可能付出整段会话持续的 Decode RTT 上浮、跨境数据合规风险与故障集中度的代价。因此至少四类场景须"弃亲和"或降低亲和权重：

1. **持续 RTT 代价超过一次性 Prefill 收益**：亲和命中省的是 Prefill（一次性），但 Decode 逐 token 持续发生，远端 Decode 的 RTT 会拖累整个会话节奏（§1.5），须按会话预计长度折算；
2. **跨境合规 / 数据主权约束**：会话 KV 与请求须在合规地域处理，跨境亲和路由让位于合规；
3. **驻留节点故障或过载**：熔断切换至就近节点，缺失部分按 §4 博弈决策（拉取或重算）；
4. **首次请求（无历史会话）**：退化为地理就近 + 共享模块预热命中（§3）。

关键在于**降级代价的量化**：假设会话上下文 100K，因上述任一原因弃亲和、切换到已预热共享模块 A/B 的备选节点，缺失的只是动态部分。以 §6.4 流程 A 的参数为例，缺失 8K token 本地重算仅需 8K ÷ 10K token/s ≈ **0.8 秒**——与正常路径的秒级 TTFT 同量级。

> **亲和性的经济学**：亲和是权重最高的因子但绝非绝对命令。命中亲和省下的是 GB 级跨域传输（一次性 80 s / 8 GB 档），丢失亲和付出的是亚秒级增量重算；但若亲和把请求推向遥远节点，省下的 Prefill 会被持续 RTT 与合规风险抵消——路由必须把它们放进同一成本函数（§6.2 部件①）。
...
把 KV 从"铁板一块"解耦为"可独立寻址的微模块"，依赖位置无关缓存（Position-Independent Caching，PIC）技术的成熟。不同于仅支持"前缀从头匹配"的传统缓存，PIC 把 KV 从"位置绑定"中解放，使语义块可以**独立寻址、独立存储**——这是把一个 Agent 上下文拆成微模块、并只分发其中高共享部分的技术前提（各论文完整贡献见附录 C；PIC 在四轴分类学中的位置见附录 B 轴四）。但 PIC 只解决"位置"问题，"哪些块能拼在一起"由 **KV 依赖 DAG**（§3.3）决定，并非任意拼装。
...
> **一句话总结**：PIC 的本质是——用数学旋转解决"位置"问题，用边界修复或离线训练解决"上下文"问题，从而把"绑死在特定位置的整段缓存"解构成"可独立寻址、按依赖 DAG 装配、全球流通的模块积木"。这正是创新点二把上下文拆成 A/B/C/D 微模块、并跨节点异步分发的数学根基。
...
**重要澄清：A/B/C/D 是按业务语义（共享度）做的分类，并不等于可自由拼接的计算模块。** 因果注意力下，C 类多个 RAG 文档之间、C 与 D 对话历史之间普遍存在因果依赖（D 引用 C、文档间相互引用）。因此装配必须以 **KV 依赖 DAG（KV Dependency DAG）** 为约束：目录记录块间依赖边，只有依赖链已满足（上游块已就位）的模块才能拼接，跨模块依赖缺失时回源拉取或局部重算补齐。A/B 这类无上游依赖的"根模块"最适合异步预分发；D 依赖 C 等上游，只能在上游就位后装配。
...
- **边缘 Decode 服务**：边缘节点装载**完整模型权重**做就近 Decode——KV 命中省的是 **Prefill 算力**，而非模型装载（权重/显存/内存带宽需求不变）。其成本优势来自"就近压 RTT（§1.5）+ 免本地大 Prefill 算力池 + 可用更便宜的 Decode 专用/异构硬件"，而非"少装东西"。

三级拓扑的成本定位：

| 层级 | 角色 | 硬件画像 | 成本定位 |
|---|---|---|---|
| L2 中心源站 | Prefill 生成 + 全量 KV 版本库 | Rubin CPX/B200 级重算力池（万卡规模摊薄） | 集中承担昂贵 Prefill，规模摊薄单次成本 |
| L1 区域 POP | 大容量 KV 驻留 + 跨域传输引擎 | CXL 内存池 + NVMe，少量 GPU | 存储密集，算力需求低 |
| L0 边缘节点 | 完整权重的就近 Decode + 轻量增量 Prefill | Decode 异构/专用硬件 + SmartNIC（权重不减） | 免大 Prefill 算力池，用异构 Decode 降本，权重装载不减 |
...
flowchart TB
    ...
    subgraph TOPO["三级拓扑：算存地理分离"]
        ...
        EDGE0["L0 边缘节点 ×N<br/>完整权重就近 Decode + SmartNIC<br/>免 Prefill 算力池·异构降本"]
    end
...
| 缓存失效 | 版本/租户策略失效 | 失效键 = **KV 兼容性清单**（模型/结构、tokenizer/RoPE、精度量化/TP-PP、LoRA/系统版本）；任一维度变更即整体失效，错误不报 404，而是**静默的生成质量劣化** |
...
|---|---|---|---|---|
| ① | **全局路由网关（Global Gateway / GSLB）** | 请求接入；提取前缀/模块指纹；**缓存亲和性路由（创新点一：查目录 → 将 KV 驻留作为高权重路由因子）**；SLA 分类（实时/异步）；基于"命中率 × 端到端时延 × 成本 × 潮汐 × 信任"加权决策；**内嵌 T_net vs T_compute 动态博弈决策引擎（创新点三）**；熔断与降级（fallback to local recompute）。传统 CDN 的智能 DNS + 全局负载均衡对应物 | 一、三 | AIBrix 路由思想、GORGO |
| ② | **KV 全局目录与索引（KV Catalog）** | 前缀/**PIC 模块**→位置映射（分布式 Radix 树 + DHT 双层，模块级内容寻址）；**不可变的 KV 兼容性清单 + 依赖 DAG**——块 ID = 哈希（兼容性清单 × 分块内容），清单绑定模型权重/结构、tokenizer/prompt 模板/RoPE、精度量化/张量布局/TP-PP、LoRA/系统版本/引擎；块间因果依赖以 DAG 记录、按拓扑序装配（§3.3）；元数据与版本管理；租约发放与失效广播；冷启动影子统计与热度画像（§6.5） | 一、二 | LMCache 内容寻址、etcd/一致性哈希 |
